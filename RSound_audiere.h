...
namespace ExpertMultimediaBase {
	...
	public:
		...
		audiere::SampleSourcePtr get_source();
	protected:
	private:
	...
	public:
		...
		std::string getName();
		bool enable;
		audiere::SampleSourcePtr source; //managed elsewhere
		audiere::AudioDevicePtr device; //managed elsewhere
		std::string EmitterName; //move all instances when emitter moves
	...
	protected:
	private:
		audiere::OutputStreamPtr stream; //managed by this class
		float Position[3];
		float Velocity[3];
	...
	private:
		...
		int MaxSoundCount;
		RSoundBuffer** buffers;
		RSoundBufferInstance** instances;
		int InstanceCount;
